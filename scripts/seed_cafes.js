// scripts/seed_cafes.js
const admin = require('firebase-admin');

// Ensure we connect to the local Firestore emulator
...
async function seed() {
  console.log('Seeding cafes to local Firestore emulator...');
  ...
  await batch.commit();
  console.log(`Successfully seeded ${cafes.length} cafes!`);
  process.exit(0);
}

seed().catch(err => {
  console.error('Failed to seed cafes:', err);
  process.exit(1);
});
